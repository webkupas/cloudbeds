  ...
  <div class="cell" :class="state" :data-row="row" :data-col="col" :style="styles">
    <label class="cell-chkb">
      <input type="checkbox"
            :id="'chkb-'+col+'-'+row"
            :value="disabled"
            @change="$emit('update:disabled', $event.target.checked); beforeDisabledUpdate($event);">
      <i></i>
    </label>
    <div class="cell-text">
      <input type="text"
        :value="text"
        :disabled="disabled"
        :id="'txt-' + this.col + '-' + this.row"
        @input="$emit('update:text', $event.target.value.replace(/\D+/g, '')); addDataToSave($event)"
        @keypress="typeOnlyDigits($event);"
        @keydown="beforeTextUpdate($event.target.value.replace(/\D+/g, ''), $event)"
        @paste="beforeTextUpdate($event.target.value.replace(/\D+/g, ''), $event)">
    </div>
  ...
  methods: {
    ...
    beforeTextUpdate (val, e) {
      if (e.which === 8 || e.which === 0 || (e.which > 48 && e.which < 57)) { // checking if digit key was pressed
        ...
        if (chkb.dataset.initDisabled === undefined) {
          chkb.dataset.initDisabled = chkb.checked
        }
      }
    },
    /**
    ...
    beforeDisabledUpdate (e) {
      ...
      if (e.target.dataset.initDisabled === undefined) {
        e.target.dataset.initDisabled = !e.target.checked
      }
      /* data-init-text attribute if it does not and add initial data to store */
      let txt = document.getElementById('txt-' + this.col + '-' + this.row)
      if (txt.dataset.initText === undefined) {
        txt.dataset.initText = txt.value
      }
    ...
    addDataToSave (e) {
      ...
      this.$store.dispatch('addDataToSave', {
        x: this.col,
        y: this.row,
        text: txt.value.replace(/\D+/g, ''),
        disabled: chkb.checked,
        initDisabled: JSON.parse(chkb.dataset.initDisabled), // e.target.type === 'checkbox' ? JSON.parse(chkb.dataset.initDisabled) : JSON.parse(!chkb.dataset.initDisabled),
        initText: txt.dataset.initText !== undefined ? txt.dataset.initText : txt.value
      })
    }
  }
}
